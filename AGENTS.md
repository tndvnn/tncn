...
0. Lần dùng đầu tiên trong ngày: chạy `bash scripts/kiem-tra-cap-nhat.sh` (tự giới hạn 1 lần/24 giờ). In `CO_BAN_MOI` thì báo người dùng và, nếu đồng ý, chạy lại với `--apply` rồi đọc lại SKILL.md. Không có shell thì so `version.json` với bản trên GitHub.
...
Script duy nhất là `scripts/kiem-tra-cap-nhat.sh` (bash, chỉ chạy git fetch/curl tới github.com, không cần cài gì). Không sửa các file trong `references/` khi đang trả lời người dùng.
